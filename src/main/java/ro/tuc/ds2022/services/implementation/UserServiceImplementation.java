...
import ro.tuc.ds2022.controllers.handlers.dto.UserDetails;
...
import ro.tuc.ds2022.security.JwtTokenUtil;
...
public class UserServiceImplementation implements UserAccountService {
   ...
   public UserDetails login(LoginDto logindto)
   {
      ...
      if(ua!=null) {
          ...
          final String token = jwtTokenUtil.generateToken(ua);
          System.out.println("token:"+token);
          if (password.equals(logindto.getPassword())) {
              System.out.println("login succesful");
              Long a;
              ud.setId(ua.getId());
              ud.setDevices(ua.getDevices());
              ud.setRole(ua.getRole());
              ud.setName(ua.getName());
              ud.setToken(token);
              return ud;
          } else
          {
    ...
    public org.springframework.security.core.userdetails.UserDetails loadUserByUsername(String s) throws UsernameNotFoundException {
        return userRepository.findByRoleUsername(s).get();
    }
}
